...
# present, stile-mode would not auto-enable, and the buffer would stay
# empty -- the puppeteer's `M-x stile-demo-position-for-user' search
...
# find-file-hook -> stile-maybe-enable, the sidecar already exists and
# stile-mode (and auto-revert) auto-enable in the buffer.
