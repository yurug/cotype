...
## Scope
What the *caller* must do. The behaviour of `cotype` itself is in `algorithms.md`. Any caller that follows these protocols inherits the guarantees in `../properties/functional.md` (P1, P4, P15 in particular).
...
SAVE:
  response = cotype save FILE --base-sha base_sha --actor editor < buffer
...
The editor MUST NOT write `FILE` directly while it is under `cotype` management.
...
cotype save FILE --base-sha response.base_sha --actor process < output
```
...
cotype open FILE           # captures bytes B (possibly != A) as base
edit the bytes you read   # producing C, derived from A, not from B
cotype save FILE --base-sha B --actor X < C
```

`cotype` cannot detect this misuse. The caller asserts via `--base-sha B` that it edited from B, but it actually edited from A. If A != B, the resulting save violates the spirit of P1 even though the byte-level checks pass.
...
> The forbidden pattern is easy to fall into in shell pipelines (`cat FILE | edit | cotype save ...`). Always pipe from `base_path`, not from `FILE`, when going through `cotype`.

## Related files
- `algorithms.md` -- what `cotype` actually does on each call
