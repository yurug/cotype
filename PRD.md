...
### M1: Storage and hashing

Implement sidecar discovery, SHA-256 hashing, base snapshot storage, and JSON output.

### M2: Locking and atomic write

Implement advisory sidecar `flock`, temp-file write, fsync, and atomic rename.

### M3: Direct save path

Implement `init`, `open`, and direct `save` when current hash equals base hash.

### M4: 3-way merge path

Implement merge for stale bases using either a deterministic internal merge or a carefully wrapped `diff3`.

### M5: Conflict artifacts

Implement conflict directories, pending conflict state, and `resolve`.

### M6: Conformance tests

Add tests for all acceptance criteria, including concurrent process tests.

## 15. Future work, explicitly not v0
